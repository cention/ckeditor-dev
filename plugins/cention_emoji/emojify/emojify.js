(function(root, factory) {
	...
	var emojify = (function() {
		...
		function insertEmojicon(args) {
			...
			if(args.replacer) {
				emojiElement = args.replacer.apply({config: defaultConfig},
		...
		function emojifyString (htmlString, replacer) {
			...
			return htmlString.replace(emojiMegaRe, function() {
				...
				if(emojiName) {
					return replacer.apply({config: defaultConfig},
		...
		function isInBlacklist(tag, blacklist) {
		...
		function run(el, replacer, cke) {
			...
			if(typeof win.document.createTreeWalker !== 'undefined' && false) {
				var nodeIterator = win.document.createTreeWalker(el,
					...
					function(node) {
						...
						if(isInBlacklist(node.tagName, defaultConfig.blacklist.elements) ||
							node.tagName === "svg" ||
							isInBlacklist(node.className, defaultConfig.blacklist.classes)) {
				...
				treeTraverse(el, function(node) {
					if((typeof node.tagName !== 'undefined' &&
						isInBlacklist(node.tagName, defaultConfig.blacklist.elements)) ||
