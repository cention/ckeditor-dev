...
CKEDITOR.dialog.add( 'cention_avatar', function( editor ) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		helpers = CKEDITOR.plugins.cention_avatar,
	...
	function onLoadLockReset() {
		...
		if ( lockButton ) {
			...
			dialog.addFocusable( lockButton);
		...
		if ( resetButton ) {
			// Consider that there's an additional focusable field
			// in the dialog when the "browse" button is visible.
			dialog.addFocusable( resetButton );
	...
	var srcBoxChildren = [
			{ //hidden and added predefined src folowing avatar photo
				id: 'src',
				type: 'text',
